...
RaySkillKit now ships with a compact catalogue of nine skills that blend legacy validation fixtures with the new travel and retail packs:
...
- `retail_wtp_radar` – Willingness-to-Pay Radar for retail pricing (retail commerce)
- `retail_capsule_gaps` – Wardrobe Capsule Gap Analyzer (retail recommendations)
- `retail_minute_meal` – Quick Meal Prep Time Estimator (retail meal kits)

Model and stats artifacts are published under `rayskillkit/skills/{models,stats}/{travel,retail}`, so downstream tooling and release scripts can resolve the paths without additional configuration.

### Training and Exporting Retail Models

To train and export a retail dataset to quantized ONNX format:

```bash
# From the sdk-python directory
cd sdk-python

# Export WTP Radar model
python -m sdk_python.skill_template.export_onnx \
  --dataset rt_wtp_radar \
  --skill-id retail_wtp_radar \
  --output-dir ../rayskillkit/skills/models/retail \
  --train-samples 512 \
  --eval-samples 128 \
  --epochs 100 \
  --seed 42

# Export Capsule Gaps model
python -m sdk_python.skill_template.export_onnx \
  --dataset rt_capsule_gaps \
  --skill-id retail_capsule_gaps \
  --output-dir ../rayskillkit/skills/models/retail \
  --train-samples 512 \
  --eval-samples 128 \
  --epochs 100 \
...
# Export Minute Meal model
python -m sdk_python.skill_template.export_onnx \
  --dataset rt_minute_meal \
  --skill-id retail_minute_meal \
  --output-dir ../rayskillkit/skills/models/retail \
  --train-samples 512 \
  --eval-samples 128 \
  --epochs 100 \
...
After export, move the generated `*_stats.json` files from the models directory to `rayskillkit/skills/stats/retail/`.

The exported models are quantized to INT8 format and include:
- ONNX model file (`*_int8.onnx`)
- Statistics file with training metrics and example y-form outputs (`*_stats.json`)

All retail datasets support the same `load_synthesized_dataset` and `load_y_form_parser` interfaces as the travel datasets.
